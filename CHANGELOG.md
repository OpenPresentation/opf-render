...
- FF-22b: preview every kept classic chart type as the native construct opf-pptx exports. Chart ids resolve like the exporter: the 26 kept core ids, the 50 deprecated ids to their replacement (they render identically), `donut` to `doughnut`; other ids keep the legacy single-series preview. Column/bar render clustered, stacked (positive and negative values stack separately) and 100% stacked (0-100% axis, `%` tick labels); bar charts put the first category and series at the bottom, like PowerPoint. Line and stacked line (cumulative) render with or without circle markers; area, stacked area and 100% area are filled. Pie and doughnut plot the first series only, one colour per category, clockwise from 12 o'clock, doughnut hole 50%. Scatter uses a numeric X axis from `columns[1]` and Y series from `columns[2..]`, markers only. Radar, radar with markers and filled radar are drawn on a web grid. Value axes use Office-like 1/2/5 automatic scaling; legends sit on the right (PptxGenJS `legendPos r`) when there is more than one series, and per category for pie and doughnut. Series colours use the opf-pptx `CHART_COLORS` order with the same surface-contrast adjustment. Every mark and label keeps a `data-opf-path`; traced chart groups carry `data-opf-chart` with the resolved id. Chartex types (treemap, histogram, pareto, box-and-whisker, waterfall, funnel, map) still use the legacy preview. 115 of 805 golden rasters change, all of them chart slides; see `test/golden/README.md`. No package version change.
