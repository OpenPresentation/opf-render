...
const full=[];
for(const key of [changed[1],changed[9],changed[20],changed[107],changed[183],changed[184]]){
...
await writeFile(path.join(out,'report.json'),JSON.stringify({source:candidate.source,total:keys.length,changed:changed.length,unchanged:keys.length-changed.length,previousBaseline:'test/golden/opf-examples-png.base-fonts.sha256.json',previousRendererCommit:'44ff76fe19ae9de80d2aaa962aa7e43fabce02bc',verifierSha256:hash(await readFile(new URL(import.meta.url))),sheets,full},null,2)+'\n');
