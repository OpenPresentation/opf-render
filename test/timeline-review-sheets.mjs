...
await writeFile(path.join(out,'report.json'),JSON.stringify({source:candidate.source,total:keys.length,changed:changed.length,unchanged:keys.length-changed.length,previousBaseline:'test/golden/opf-examples-png.readability.sha256.json',previousRendererCommit:'53b90878b8a9610117ac5191ebfaaa549cc2f3ea',candidateCoreCommit:'cc4b5245ba1c197746a2c307b78ff58f45df02cc',previousCoreCandidate:JSON.parse(await readFile(new URL('../artifacts/timeline-before-runtime/before.json',import.meta.url))),verifierSha256:hash(await readFile(new URL(import.meta.url))),sheets,full},null,2)+'\n');
