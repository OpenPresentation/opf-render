...
import {renderSvgDeck as renderBefore} from '../artifacts/readability-before-runtime/dist/index.js';
const before=path.resolve(process.argv[2]??'artifacts/source-whitespace-golden-after'),after=path.resolve(process.argv[3]??'artifacts/readability-golden-after'),out=path.resolve(process.argv[4]??'docs/evidence/readability-corpus-review');
...
await writeFile(path.join(out,'report.json'),JSON.stringify({source:candidate.source,total:keys.length,changed:changed.length,unchanged:keys.length-changed.length,previousBaseline:'test/golden/opf-examples-png.source-whitespace.sha256.json',previousRendererCommit:'c7a7387f6e27cc21eeeec3015492afe289ec4ce3',candidateCoreCommit:'ade2a4f0a8afa93f546a3b1e18f2ed23b84d0ac7',previousCoreCandidate:JSON.parse(await readFile(new URL('../artifacts/readability-before-runtime/before.json',import.meta.url))),verifierSha256:hash(await readFile(new URL(import.meta.url))),sheets,full},null,2)+'\n');
